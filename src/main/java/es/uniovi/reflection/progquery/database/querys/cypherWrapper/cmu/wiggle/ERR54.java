...
import es.uniovi.reflection.progquery.database.querys.services.MethodInvocationServicesWiggle;
import es.uniovi.reflection.progquery.database.querys.services.PDGServicesWiggle;
import es.uniovi.reflection.progquery.database.querys.services.TypeServicesWiggle;

public class ERR54 extends AbstractQuery {
	...
	protected void initiate() {
		...
		clauses = new Clause[] { (Clause) getCFGServices().getCFGSuccesorsOf(new NodeVar("newStat"), ""),
				...
				new MatchClause(true,
						"(non_declaredEx)<-[:HAS_CLASS_EXTENDS]-(superDeclaredEx),hier=(declaredEx)-[:IS_SUBTYPE_OF*0..]->(superDeclaredEx)"
						+ ", (declaredEx)<-[:IS_SUBTYPE_OF*0..]-(subTypeDeclared)"
								+ " WHERE non_declaredEx.actualType IN EXTRACT(row IN exHier | row[0])" + ""
										+ " WITH  DISTINCT"
										+ " autocloseableTypeNames,sucessors, mThrowsInfo, "
										+ " exHier, "
										+ " [declaredEx.fullyQualifiedName"
										+ ",EXTRACT(x IN TAIL(NODES(hier)) | x.fullyQualifiedName)+[non_declaredEx.actualType]+ FILTER(hier IN exHier WHERE hier[0]=non_declaredEx.actualType)[0][1]"
										+ ",EXTRACT(x IN COLLECT(subTypeDeclared) | x.fullyQualifiedName) ] as declaredHier"
										+ " WITH DISTINCT autocloseableTypeNames,sucessors, mThrowsInfo, exHier+COLLECT(declaredHier) as exHier"),
				new ClauseImpl(
						"MATCH(inv)<-[:ARRAYACCESS_EXPR|ARRAYACCESS_INDEX|ASSIGNMENT_LHS|ASSIGNMENT_RHS|BINOP_LHS|BINOP_RHS|CAST_ENCLOSES|COMPOUND_ASSIGNMENT_LHS|COMPOUND_ASSIGNMENT_RHS|CONDITIONAL_CONDITION|CONDITIONAL_THEN|CONDITIONAL_ELSE|HAS_VARIABLEDECL_INIT|INSTANCEOF_EXPR|PARENTHESIZED_ENCLOSES|MEMBER_SELECT_EXPR|METHODINVOCATION_ARGUMENTS|METHODINVOCATION_METHOD_SELECT|NEW_CLASS_ARGUMENTS|NEWARRAY_INIT|NEWARRAY_DIMENSION|UNARY_ENCLOSES*0..]-()<-[:ASSERT_CONDITION|DOWHILE_CONDITION|EXPR_ENCLOSES|FOREACH_EXPR|FORLOOP_CONDITION|HAS_VARIABLEDECL_INIT|IF_CONDITION|RETURN_EXPR|SWITCH_EXPR|SYNCHRONIZED_EXPR|THROW_EXPR|WHILE_CONDITION]-(stat)<-[:CASE_STATEMENTS|CATCH_BLOCK|CATCH_PARAM|ENCLOSES|FOREACH_STATEMENT|FOREACH_VAR|FORLOOP_INIT|FORLOOP_STATEMENT|FORLOOP_UPDATE|HAS_METHODDECL_PARAMETERS|IF_THEN|IF_ELSE|LABELED_STATEMENT|SWITCH_ENCLOSES_CASES|SYNCHRONIZED_BLOCK|TRY_BLOCK|TRY_CATCH|TRY_FINALLY|TRY_RESOURCES*]-()<-[:HAS_METHODDECL_BODY]-({nodeType:'JCMethodDecl'})<-[:DECLARES_METHOD]-(callerEncType)\r\n"
								...
								+ " WITH exHier, autocloseableTypeNames,sucessors, mThrowsInfo,stat,inv,invMember,arg, CASE WHEN invMember IS NULL THEN inv.actualType ELSE CASE WHEN invMember.nodeType='JCIdent' THEN callerEncType.fullyQualifiedName ELSE memberSelection.actualType END END as callerEncTypeName ORDER BY ID(arg)"
								+ " WITH  DISTINCT " + "exHier,autocloseableTypeNames,sucessors, mThrowsInfo,"
				...
				new SimpleWithClause("DISTINCT " 
				+ "exHier,autocloseableTypeNames,sucessors,mThrowsInfo,"
...
new SimpleWithClause(
		...
		"stat,"
		+ "REDUCE(s=[], x IN "
		+ "EXTRACT(invSignature IN invInfoInStats | "
		+ " FILTER(throwSignature IN mThrowsInfo WHERE throwSignature[0]=invSignature[0] AND throwSignature[1]=invSignature[1] AND throwSignature[2]=invSignature[2])[0][3] )"
		+ "| CASE WHEN x IS NULL THEN s ELSE s+x END ) "
		+ "as throws "
		)
...
, 
				new SimpleWithClause(
						"DISTINCT "
						+ "exHier, autocloseableTypeNames,sucessors,"
						+ " FILTER(x IN COLLECT( [stat,throws]) WHERE SIZE(x[1])>0) as statsMayThrow"),
			new MatchClause(true,
						"(stat)-[:THROW_EXPR]->(throwExpr)  WITH DISTINCT exHier,autocloseableTypeNames,sucessors, statsMayThrow+COLLECT([stat,throwExpr.actualType]) as statsMayThrow"),
				new MatchClause(true,
						"(stat{nodeType:'JCAssert'}) WITH DISTINCT "
						+ "exHier, autocloseableTypeNames,sucessors,"
						+ " statsMayThrow+COLLECT([stat,'java.lang.AssertionError']) as statsMayThrow")

				,new MatchClause(PDGServicesWiggle.WIGGLE.getIdsAndVarDeclarations(new NodeVar("id"),
						"exHier, autocloseableTypeNames,sucessors,statsMayThrow ")),
				new SimpleWithClause("exHier, autocloseableTypeNames,sucessors,statsMayThrow ,id, varDec, varDec.actualType IN autocloseableTypeNames as isVarClos"),
				new WhereClause("isVarClos"),
				new SimpleWithClause(
						"DISTINCT exHier,autocloseableTypeNames,sucessors,statsMayThrow, varDec as dec, COLLECT(DISTINCT id) as ids"),
				// new ReturnClause("DISTINCT dec, ids"),
				new MatchClause(true,
						"  (dec)<-[r:TRY_RESOURCES]-() WITH DISTINCT exHier,sucessors,statsMayThrow, dec,ids,r WHERE r IS NULL"),
				...
				new SimpleWithClause(" DISTINCT " + "exHier, sucessors,statsMayThrow,ids,  " + "dec, COLLECT(stat) as mods "),

				new MatchClause(true, new MatchImpl("(dec)-[r:HAS_VARIABLEDECL_INIT]->()")),
				new SimpleWithClause("DISTINCT " + "exHier, sucessors, statsMayThrow,"
				...
				new SimpleWithClause("DISTINCT exHier,sucessors,statsMayThrow, dec, mod, COLLECT(closeStat) as closeStmts"),
				new SimpleWithClause(
						" exHier,sucessors,"+
						 " dec, mod,closeStmts, FILTER( stat IN statsMayThrow WHERE NOT stat[0] IN closeStmts AND stat[0] IN FILTER(x IN sucessors WHERE x[0]=mod)[0][1]"
								+ " AND ANY(closeStat IN closeStmts WHERE closeStat IN FILTER(x IN sucessors WHERE x[0]=stat[0])[0][1]) ) as prevStats"),
				new UnwindClause("prevStats", "prevStat"),
				// new ReturnClause("DISTINCT FILTER(x IN sucessors WHERE
				// x[0].nodeType='JCCatch' )")
new SimpleWithClause("exHier,sucessors,dec, mod,closeStmts, prevStat[0] as prevStat, prevStat[1] as throwTypes"),
				new MatchClause(true,
				...
				new MatchClause(true, "(try)-[:TRY_CATCH ]->(catch)-[:CATCH_PARAM]->(catchVar)"),
				
				new SimpleWithClause(
						"DISTINCT "
						+ "sucessors,throwTypes,"
						+ " dec, mod, prevStat,closeStmts, try,  catch, FILTER(catchHier IN exHier WHERE catchHier[0]=catchVar.actualType)[0] as catchHier "),
			new UnwindClause("throwTypes", "throwType"),
				new SimpleWithClause(
						"DISTINCT "
						+ "sucessors,"
						+ "  dec, mod, prevStat,closeStmts, try, catch,throwType, throwType IN catchHier[1] as maybeCaught,throwType IN catchHier[2] as alwaysCaught ORDER BY catch.position "),
...
//						"DISTINCT sucessors,dec, mod, prevStat,closeStmts, try, REDUCE(l=[],catch IN COLLECT(catch) | l+FILTER(x IN sucessors WHERE x[0]=catch)[0][1] ) as catchSucs"),
				...
				new SimpleWithClause(
					 "sucessors,"
						+ " dec, mod, prevStat,closeStmts,throwType, FILTER( x IN COLLECT([catch,maybeCaught,alwaysCaught]) WHERE NOT x[0] IS NULL) as catchInfo, COLLECT(finallyStat) as finallySucs")
				,	
				new SimpleWithClause("dec, mod, prevStat,closeStmts, throwType, "
						+ "REDUCE(l=[],catchSucs IN EXTRACT( x IN FILTER(index IN RANGE(0,SIZE(catchInfo)-1) WHERE (catchInfo[index][1] OR catchInfo[index][2]) AND index<=FILTER(i IN RANGE(0,SIZE(catchInfo)-1) WHERE catchInfo[i][2])[0]) "
						+ " | FILTER(sucInfo IN sucessors WHERE catchInfo[x][0]=sucInfo[0])[0][1] )"
						+ "| l+catchSucs) +finallySucs as stmsReacheableAfterEx"),
...
//						" DISTINCT dec, mod, prevStat,closeStmts, try, CASE WHEN SIZE(catchSucs)=0 THEN  COLLECT(finallyStat) ELSE catchSucs END as stmsReacheableAfterEx"),
				...
				new SimpleWithClause("DISTINCT COLLECT(warning) as warnings"),
				new MatchClause(false, "()-[r:IS_SUBTYPE_OF]->() DELETE r WITH DISTINCT warnings RETURN warnings")
