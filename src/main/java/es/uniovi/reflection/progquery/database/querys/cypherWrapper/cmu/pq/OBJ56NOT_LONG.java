...
public class OBJ56NOT_LONG extends AbstractQuery {

	public OBJ56NOT_LONG(boolean isProgQuery) {
		super(isProgQuery);
	...
	protected void initiate() {
		...
		clauses = new Clause[] {
				...
				new UnwindClause("CASE WHEN p IS NULL THEN [] ELSE NODES(p) END", "nodeInP"),
				new SimpleWithClause(
						"enclosingType, field, method, typeDec,p, nodeInP"),
				...
				new SimpleWithClause(
						"enclosingType, field, method, typeDec, nodeInP,p, setMethod, COLLECT(DISTINCT fType) as mutableDependencies"),
				new MatchClause(true,
						new Path(new NodeVar("nodeInP"),
				...
				new SimpleWithClause(
						"enclosingType, field, method, typeDec, nodeInP,p, SIZE( COLLECT( setMethod))>0 as isThereASetMethod, mutableDependencies, COLLECT(DISTINCT [f,fType]) as otherMutableDependencies "),
				new SimpleWithClause(
						"enclosingType, field, method, typeDec, nodeInP,p, isThereASetMethod, mutableDependencies, "
