...
public class OBJ56_SIMPLIFIED_PREV extends AbstractQuery {

	public OBJ56_SIMPLIFIED_PREV() {
		super(true);
		...
		clauses = new Clause[] {
				...
				new MatchClause(new RelationshipImpl(new NodeVar("field"), new NodeVar("fieldType"),
						...
						new Path(new NodeVar("fieldType"), "accessibleMembers",
								Pair.create(new EdgeImpl(Cardinalidad.MIN_TO_INF(0), RelationTypes.DECLARES_FIELD,
										TypeRelations.ITS_TYPE_IS, TypeRelations.INHERITS_FIELD),
				...
				new MatchClause(true,
						getStatementServices()
								...
								.append(Pair.create(new EdgeImpl(EdgeDirection.OUTGOING, RelationTypes.DECLARES_METHOD,
										TypeRelations.INHERITS_METHOD), new NodeVar("accessibleType")))),
				new WhereClause(
						"ID(accesibleField)=ID(accMemberAndPrev[1]) AND ID(accessibleType)=ID(accMemberAndPrev[0])"),
				new SimpleWithClause(
						" field,enclosingType, method,publicGetters,accMemberAndPrev, accessibleMembers, accessibleMember, CASE WHEN method IS NULL THEN [NULL] ELSE COLLECT(method) END as gettersForCurrentMember"),

				new SimpleWithClause(
						"field,enclosingType,publicGetters,accessibleMembers,accessibleMember,accMemberAndPrev+ gettersForCurrentMember as accMemberOrType"),

				new SimpleWithClause(
						"field,enclosingType,publicGetters,accessibleMembers,accessibleMember, COLLECT( CASE WHEN accMemberOrType[1]:ATTR_DEC THEN NOT accMemberOrType[1].isStatic AND (accMemberOrType[1].accessLevel='public' OR NOT accMemberOrType[2] is NULL) ELSE CASE WHEN accMemberOrType[1]:ARRAY_TYPE THEN TRUE ELSE accMemberOrType[1].accessLevel='public' END END + accMemberOrType) as accMemberOrTypes"),
				new WhereClause("ALL( accMemberOrType IN accMemberOrTypes WHERE accMemberOrType[0])"),
				new SimpleWithClause("DISTINCT field, enclosingType, publicGetters,  accessibleMember "),
				new MatchClause(true, new RelationshipImpl(new NodeVar("accessibleMember"),
						new CompleteNode("mutator", NodeTypes.METHOD_DEF, Pair.create("accessLevel", "public")),
						RelationTypes.DECLARES_METHOD, TypeRelations.INHERITS_METHOD)
								.append(Pair.create(
										new EdgeImpl(EdgeDirection.OUTGOING, PDGRelationTypes.STATE_MAY_BE_MODIFIED_BY,
												PDGRelationTypes.STATE_MODIFIED_BY),
										new CompleteNode(NodeTypes.THIS_REF)))),

				new SimpleWithClause(
						"field, enclosingType, publicGetters, accessibleMember, COLLECT(mutator) as mutators"),
				new SimpleWithClause(
						"field,enclosingType, mutators,publicGetters, EXTRACT(accField IN FILTER(accMember IN COLLECT(accessibleMember) WHERE accMember:ATTR_DEC AND NOT accMember.isFinal AND accMember.accessLevel='public') | 'PUBLIC NON-FINAL FIELD '+accField.name+'( line'+accField.lineNumber+')') as externallyMutableFields"),

				new WhereClause(
						"SIZE(mutators)>0 OR SIZE(externallyMutableFields)>0"),
				new ReturnClause(
						" 'Warning[OBJ-56] Field ' +field.name+' declared in line ' +field.lineNumber+' in class '+enclosingType.fullyQualifiedName+' is not public, but it is exposed in public methods such as '+ publicGetters+'. The problem is that there is at least one member ( like '+EXTRACT(mutator IN mutators | 'MUTATOR METHOD '+mutator.fullyQualifiedName+'( line '+mutator.lineNumber +')')+externallyMutableFields+')that can be accessed by a client to change the state of the field '+field.name")
				// new ReturnClause(
