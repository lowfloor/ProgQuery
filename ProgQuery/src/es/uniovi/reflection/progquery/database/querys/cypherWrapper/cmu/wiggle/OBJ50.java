...
import es.uniovi.reflection.progquery.database.querys.services.FieldServicesWiggle;
import es.uniovi.reflection.progquery.database.relations.RelationTypesWiggle;

public class OBJ50 extends AbstractQuery {
...
//AQUI LO HAREMOS
		...
		+ getStatementServices().getMethodFromStatement(getExpressionServices().getStatementFromExp(new NodeVar("modification")), new MatchImpl("(method)-[:HAS_METHODDECL_MODIFIERS]->(methodMod)")).matchToString()					
				 ...
				 new SimpleWithClause("DISTINCT notInitMethods,identss,idDecType, invIds,method as mutator,lhs, firstId, FILTER(idP IN idDecType WHERE idP[0]=firstId)[0][1] as pType "),

				 new WhereClause("lhs<>firstId OR pType=0"),
 new SimpleWithClause("DISTINCT  idDecType,notInitMethods,identss, invIds, mutator, COLLECT(DISTINCT pType) as pTypes "), 
 new MatchClause(false,"invTrace=(anyMethod{nodeType:'JCMethodDecl'})-[:CALLS | :HAS_DEC*0..]->(mutator)"),
 new SimpleWithClause("DISTINCT  idDecType,notInitMethods,identss, anyMethod,mutator, EXTRACT(inv IN FILTER(inv IN NODES(invTrace) WHERE inv.nodeType='JCMethodInvocation') | FILTER(invId IN invIds WHERE invId[0]=inv)[0][1]) as invTracePlusIdDec,  pTypes"), 
 new WhereClause("ALL(inv IN invTracePlusIdDec WHERE NOT inv IS NULL)"),
 new SimpleWithClause("idDecType, notInitMethods,identss, anyMethod, "
 ...
 new SimpleWithClause("DISTINCT idDecType, notInitMethods,identss, anyMethod, REDUCE (s=[], x IN COLLECT(mutableDecs)  | s+x) as mutableDecs")
 , new SimpleWithClause("DISTINCT idDecType, notInitMethods,identss, COLLECT([anyMethod, mutableDecs]) as mutatorMethods "),
...
//				new SimpleWithClause("p[0] as dec, p[1] as ids, identss"),
	...
	+ getStatementServices().getMethodFromStatement(getExpressionServices().getStatementFromExp(new NodeVar("modification")), new NodeVar("method")).matchToString()+"<-[:DECLARES_METHOD]-(encClass)"					
						),
				new WhereClause(
//						"NOT (NOT method IN notInitMethods AND ) AND "+
				...
				new SimpleWithClause("DISTINCT idDecType, mutatorMethods, notInitMethods,identss, varDecl,COLLECT([firstId, method, modification,encClass]) as stateMods"),
				...
				new MatchClause(true, "(invDec)<-[:HAS_DEC]-(inv)-[:METHODINVOCATION_METHOD_SELECT]->()-[ :MEMBER_SELECT_EXPR | :ARRAYACCESS_EXPR*1..]->(id)-[:MEMBER_SELECT_EXPR | :ARRAYACCESS_EXPR*0..]->(firstId{nodeType:'JCIdent'}),"
						+ "(inv)<-[:CALLS]-(method)<-[:DECLARES_METHOD]-(encClass) "
...
, new WhereClause("id IN EXTRACT(x IN FILTER(idDec IN identss WHERE idDec[1]=varDecl)| x[0]) "
		...
		, new SimpleWithClause("DISTINCT idDecType, mutatorMethods, notInitMethods,identss, varDecl,stateMods + COLLECT([firstId, method, inv,encClass]) as stateMods "),
...
//				+ " AND 0 IN  FILTER(methodAndMutDecs IN mutatorMethods WHERE invDec=methodAndMutDecs[0])[0][1]"
		...
		new SimpleWithClause("idDecType, mutatorMethods,notInitMethods, varDecl,stateMods,method, invDec,inv,encClass, ID(arg) as order, firstId ORDER BY order"),
		new SimpleWithClause("idDecType, mutatorMethods,notInitMethods, varDecl,stateMods,method, invDec, COLLECT([firstId, method, inv,encClass]) as argInfo ")
		,
		new SimpleWithClause("DISTINCT varDecl,"
				+ "EXTRACT(sMod IN "
				+ "FILTER(refinedSMod IN "
				+ "EXTRACT(stateMod IN "
				+ "stateMods+ EXTRACT( argIndex IN FILTER(i IN RANGE(0,SIZE(argInfo)-1) WHERE (i+1) IN  FILTER(methodAndMutDecs IN mutatorMethods WHERE invDec=methodAndMutDecs[0])[0][1]) | argInfo[argIndex])"
				+ "|[FILTER(idDecT IN idDecType WHERE idDecT[0]=stateMod[0])[0][1], stateMod[1],stateMod[2],stateMod[3]]) "
				+ "WHERE refinedSMod[1] IN notInitMethods OR (refinedSMod[0] IS NULL AND NOT refinedSMod[1] IS NULL) OR refinedSMod[0]<>0)"
				+ "| sMod[2].nodeType +'(line '+sMod[2].lineNumber+', class '+sMod[3].fullyQualifiedName+')') as stateMods"
				)
		, new MatchClause(getStatementServices().getEnclosingClassFromDeclaration(new NodeVar("varDecl"))),
				new SimpleWithClause(
						"DISTINCT EXTRACT( varAndMods IN FILTER(x IN COLLECT([varDecl, stateMods, enclClass]) WHERE SIZE(x[1])>0)"
				+ "|'Warning [CMU-OBJ50] Declaration with name '+ varAndMods[0].name+ '( in line ' + varAndMods[0].lineNumber +', class ' +varAndMods[2].fullyQualifiedName+ ') is not actually final, only the reference. Concretely, '+ varAndMods[0].name +' may be mutated in '+varAndMods[1] )"
				+ " as res"
				)

				, new ClauseImpl("MATCH ()-[r]->() WHERE r.created DELETE r "),
				 new ReturnClause("DISTINCT res")
		};

	}

	public static void main(String[] args) {
		System.out.println(new OBJ50().queryToString());
	}
}
