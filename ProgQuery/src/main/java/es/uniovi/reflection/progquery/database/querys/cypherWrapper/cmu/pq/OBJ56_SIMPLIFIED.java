...
public class OBJ56_SIMPLIFIED extends AbstractQuery {

	public OBJ56_SIMPLIFIED() {
		super(true);
	}
	...
	protected void initiate() {
		...
		clauses = new Clause[] {
				...
				new WhereClause("NOT field.accessLevel='public' AND (NOT field.accessLevel='protected' OR classExposingF.isFinal)"),
				...
				new MatchClause(new RelationshipImpl(new NodeVar("field"), new NodeVar("fieldType"),
						new EdgeImpl(TypeRelations.ITS_TYPE_IS))
								.append(Pair.create(
										new EdgeImpl(EdgeDirection.OUTGOING, TypeRelations.IS_SUBTYPE_EXTENDS,
												TypeRelations.IS_SUBTYPE_IMPLEMENTS)
														.setCardinalidad(Cardinalidad.MIN_TO_INF(0)),
										new NodeVar("fieldTypeOrSubtype"))),
						new Path(new NodeVar("fieldTypeOrSubtype"), "accessibleMembers",
				...
				new SimpleWithClause(
						"field,fieldType=fieldTypeOrSubtype as isFieldType,enclosingType, method,accessibleMember,fieldTypeOrSubtype, NODES(accessibleMembers) as accessibleMembers"),
				new SimpleWithClause(
				...
				// COLLECT( CASE WHEN accMemberOrType[1]:ATTR_DEC THEN NOT
				// accMemberOrType[1].isStatic AND
				// (accMemberOrType[1].accessLevel='public' OR
				// SIZE(relevantGetters)>0) ELSE CASE WHEN
				// accMemberOrType[1]:ARRAY_TYPE THEN TRUE ELSE
				// accMemberOrType[1].accessLevel='public' END END ) as
...
//						"DISTINCT field,isFieldType,fieldTypeOrSubtype, enclosingType, publicGetters,  accessibleMember,accessibleMemberPrev, ALL( isAccHere IN isAccesible WHERE isAccHere) as isExternallyAcc"),
				...
				// enclosingType, publicGetters,
				// accessibleMember,accessibleMemberPrev, isExternallyAcc"),
