...
public class WiggleExtendedCallGraph2 extends AbstractQuery {
	...
	public WiggleExtendedCallGraph2() {
		super(false);
	}
...
//				new ClauseImpl("MATCH (m{nodeType:'JCMethodDecl'})-[r:CALLS]->() DELETE r WITH DISTINCT '' as aux"),
				...
				new MatchClause(new MatchImpl(getStatementServices().getMethodFromStatement(getExpressionServices()
						...
						"<-[:DECLARES_METHOD]-(callerEncType) MERGE (inv)<-[r:CALLS]-(method) ON CREATE SET r.created=TRUE WITH method, inv, callerEncType"
						)),
...
new ClauseImpl("MERGE (inv)-[r2:HAS_DEC]->(possibleCalle) ON CREATE SET r2.created=TRUE ")
	...
	public static void main(String[] args) {
		System.out.println(new WiggleExtendedCallGraph2().queryToString());
	}
}
