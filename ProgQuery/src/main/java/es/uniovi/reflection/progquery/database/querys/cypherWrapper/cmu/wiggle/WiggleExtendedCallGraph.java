...
public class WiggleExtendedCallGraph extends AbstractQuery {
	...
	public WiggleExtendedCallGraph() {
		super(false);
	}
...
//				new ClauseImpl("MATCH (m{nodeType:'JCMethodDecl'})-[r:CALLS]->() DELETE r WITH DISTINCT '' as aux"),
				...
				new MatchClause(new MatchImpl(getStatementServices().getMethodFromStatement(getExpressionServices()
						...
						"<-[:DECLARES_METHOD]-(callerEncType) MERGE (method)-[r:CALLS]->(inv) ON CREATE SET r.created=TRUE WITH  inv, callerEncType"
						)),
				...
				new SimpleWithClause("invMember, inv,   CASE WHEN invMember.nodeType='JCIdent' THEN callerEncType ELSE otherPossibleCallerEncType END as callerEncType" )
...
,new WhereClause("NOT callerEncType IS NULL"),
								...
								new SimpleWithClause("inv, invMember,   callerEncType,encSubType, invDeclaration, p ORDER BY ID(p)"),
								
								new SimpleWithClause("inv,invMember, FILTER(argType IN SPLIT(SPLIT(SUBSTRING(invMember.actualType,1),')')[0],',') WHERE argType<>'') as argTypes,   callerEncType, invDeclaration, COLLECT(p.actualType) as pTypes"),
								new SimpleWithClause("inv,invMember,   callerEncType,argTypes, CASE WHEN SIZE(pTypes)=SIZE(argTypes) AND ALL(index IN RANGE(0,SIZE(pTypes)-1) WHERE argTypes[index]=pTypes[index]) THEN invDeclaration ELSE NULL END as invDeclaration "),
								/*
								new WhereClause("SIZE(pTypes)=SIZE(argTypes) AND ALL(index IN RANGE(0,SIZE(pTypes)-1) WHERE argTypes[index]=pTypes[index])"
										
										),*/
								new SimpleWithClause("inv, invMember,argTypes,   callerEncType, COLLECT(invDeclaration) as invDecs"),
//								new SimpleWithClause("inv, invMember,argTypes,  method, callerEncType, invDecs "),
								
								// SI el m�todo buscado no est� T, entonces buscamos el primer super que tenga el m�todo, y siempre le sumamos la lista anterior de T'::m donde T'<:T
...
new WhereClause("CASE WHEN invMember.name IN ['this','super'] THEN '<init>' ELSE invMember.name END =superInvDeclaration.name AND (callerEncType<>encSuperType OR invMember.name<>'super')"),
				...
				new SimpleWithClause("hierarchy,inv, invMember, argTypes,  callerEncType,encSuperType,superInvDeclaration, invDecs, p ORDER BY ID(p)"),
				//
				new SimpleWithClause("hierarchy,inv,invMember,argTypes,    callerEncType,encSuperType,superInvDeclaration, invDecs, COLLECT(p.actualType) as pTypes"),
				new SimpleWithClause("inv,invMember,   callerEncType, invDecs, CASE WHEN SIZE(pTypes)=SIZE(argTypes) "
...
new ClauseImpl("MERGE (inv)-[r:HAS_DEC]->(possibleCalle) ON CREATE SET r.created=TRUE ")
	...
	public static void main(String[] args) {
		System.out.println(new WiggleExtendedCallGraph().queryToString());
	}
}
