...
public class DCL53 extends AbstractQuery {
	/*
	 ...
	 */
	public DCL53(boolean isProgQuery) {
		super(isProgQuery);
	...
	protected void initiate() {
		clauses = new Clause[] { new ClauseImpl(
		OBJ56.CREATE_ALL_SUBTYPE_RELS+" WITH DISTINCT 'l' as l "),
...
//				new SimpleWithClause("typeDec,attr,COLLECT( id) as ids"),
				new MatchClause(true, new MatchImpl("(modif{nodeType:'JCAssign'})-[:ASSIGNMENT_LHS ]->(id)")),
				 ...
				 new SimpleWithClause("identss,typeDec,attr,COLLECT( id) as assignIds, ids"),
...
//					
				 ...
				 new MatchClause(true,getStatementServices().getMethodFromStatement(
					...
					new ClauseImpl(" OPTIONAL MATCH (useId)-[:MEMBER_SELECT_EXPR]->(memberSelectExprUse)"
							+ " WITH identss,attr, assignId,  useId, useStat,stat, method, memberSelectExprUse, className,FILTER(x IN identss WHERE memberSelectExprUse IN x[1])[0][0] as varDec"),
					new MatchClause(true, new MatchImpl("(assignId)-[:MEMBER_SELECT_EXPR]->(memberSelectModif)")),
					new SimpleWithClause(
							"attr, useStat,stat, method, memberSelectExprUse, varDec,memberSelectModif,className,FILTER(x IN identss WHERE memberSelectModif IN x[1])[0][0] as modifIdvarDec,useId, assignId"),
...
//						
				new SimpleWithClause(
						"DISTINCT 'Warning [CMU-DCL53] You must minimize the scope of the varaibles. You can minimize the scope of the attribute '+attr.name+' in class '+className + ' by transforming it into a local variable (as everytime its value is used in a method, there is a previous unconditional assignment).' as warning "),
				new SimpleWithClause(" DISTINCT COLLECT(warning) as warningList "),
						new ClauseImpl("MATCH ()-[r{created:true}]->() DELETE r "),
...
//						, new ReturnClause(" warningList")
	...
	public static void main(String[] args) {
		System.out.println(new DCL53(false).queryToString());
	}
